...
- `snippets/calendly-modal.liquid`: **the booking path is split by device as of 2026-09-08, and booking speed now outranks PageSpeed here.** Touch layouts (`(pointer: coarse)` or `max-width: 768px`) skip the modal entirely and navigate straight to Calendly via `href`. Desktop keeps the iframe modal and preloads it on `pointerenter`/`focus` *and* on the `load` event.

  The owner set that priority explicitly, so do not "restore" hover-only preloading or re-wrap the mobile path in the modal on PageSpeed grounds. The data behind it: over Aug 11 to Sep 7 2026, GA4 recorded 22 users reaching the Calendly page, 5 picking a day, 4 booking. Almost the entire loss is at step one, which is where the cold-tap spinner below lives.

  Why mobile navigates rather than preloads: the modal loads Calendly's ~2.5MB **embed** bundle into an iframe on top of an already-loaded page, while navigation hits Calendly's much smaller standalone booking page. No preload strategy beats not loading it, and this costs nothing on the mobile PSI run because nothing extra loads at all.

  **The old rule still holds for mobile**: do not add any page-load-triggered preload on a touch layout. Tried and reverted twice, once by the site owner in `0c51e5c` (2026-07-06) firing on `DOMContentLoaded`, and again in `87f0005` (2026-08-19) deferred behind a 1.5s timer plus `requestIdleCallback` with a 6s timeout, reverted in `23efcce`. Even that far out, PSI caught it: Calendly's `assets/booking/js/booking-*.js` became the single largest CPU consumer on the page at 986ms (464ms evaluation, 519ms parse), more than Google Tag Manager's entire combined footprint. Lighthouse's trace window extends well past what feels like a safe deferral. `IS_TOUCH_LAYOUT` short-circuits `preloadCalendly`, which is what keeps that promise.

  The desktop `load`-event preload is the accepted exception: desktop throttling is much lighter, and hooking `load` (not a timer) guarantees the hero image has already been fetched, so Calendly never competes with the LCP element. The 4-5 second blank spinner on a cold tap is no longer reachable on mobile, since mobile no longer opens the modal.

  Tracking note: `calendly_event_scheduled` only ever fires on desktop, since the mobile path has no iframe to receive Calendly's `postMessage`. This is fine and was accounted for. The conversion of record is `invitee_meeting_scheduled` from Calendly's own GA4 integration, which fires on both paths, and UTM params ride along in the navigation URL so `booking_source` attribution survives. The custom pixel that used to forward `calendly_event_scheduled`/`calendly_modal_open` to GA4 was removed 2026-09-16 (it duplicated measurement on `G-SKJK8YPREK`); those two events no longer reach GA4 by any path, and that's accepted since `invitee_meeting_scheduled` already covers the conversion. See `docs/ga4-custom-pixel.js` for the historical/removed setup.

  A separate Google Ads conversion (`AW-18363133778`, "Book appointment") was added 2026-09-16 directly in `layout/theme.liquid` (Ads-only `gtag`, never configured for `G-SKJK8YPREK`) and fired from the same `calendly.event_scheduled` handler in `snippets/calendly-modal.liquid`. Same desktop-only limitation as above, for the same reason.
- `snippets/belle-exit-survey.liquid` (added 2026-09-24, rendered from `belle-footer.liquid` next to `calendly-modal`): a short "why are you leaving" popup. Desktop fires on classic exit-intent (cursor leaves the top of the viewport); touch layouts have no cursor to leave with, so they use a best-effort heuristic instead (a fast upward scroll back near the top of the page after having scrolled down), which will both miss real exits and occasionally false-positive. At most once per tab (`sessionStorage`), and skipped while the Calendly modal is open. It publishes `exit_survey_shown` / `exit_survey_response` / `exit_survey_dismissed` via `Shopify.analytics.publish`, which reach GA4 through a **new, separate** Shopify custom pixel documented in `docs/ga4-exit-survey-pixel.js` - do not confuse this with the removed `docs/ga4-custom-pixel.js` below, and do not merge the two pixels. None of these three events are conversions; don't mark them as GA4 key events.

  On a chosen reason, it also emails the response via **EmailJS** (client-side send, no backend) to sales@bellesperformancetennis.com. **Do not try routing this through Shopify's native `{% form 'contact' %}` instead** - that was tried and confirmed broken 2026-09-24: a silent/background submission gets rejected with `"Missing CAPTCHA token"`, because Shopify's contact form requires a token that only its own bot-protection script can generate, which only runs against a genuinely visible, interacted-with form. EmailJS is set up and live: `EMAILJS_SERVICE_ID` (`service_kn1nejo`, Gmail service connected as sales@bellesperformancetennis.com) and `EMAILJS_TEMPLATE_ID` (`template_o02snl7`, the "Contact Us" template, To Email fixed to that same address) near the top of the snippet's `<script>` are real values, verified with a live 200/OK send 2026-09-24, not placeholders. The EmailJS account itself, its templates, and the connected Gmail live outside this repo - if the send ever needs debugging, check the EmailJS dashboard (emailjs.com), not just the theme code. Don't repoint `EMAILJS_TEMPLATE_ID` at the account's other template, "Auto-Reply" (`template_2lr0lnf`) - that one is meant to reply to a form submitter's own address, and this survey never collects a visitor email.

  After picking a reason, the visitor lands on an optional "want me to follow up?" step (name, email, zip code, all optional, one "Send" button that submits whatever was filled in) before the response actually goes out - added so the fast one-click path still exists but people willing to leave contact info can. Those three fields go **only** to the EmailJS send, never to the GA4 pixel (`track()` only ever sends `reason`/`reason_detail`/`page_path` - see the comment directly above `respond()` in the snippet; Google's terms prohibit sending PII to GA4, and there's no reporting need for it there anyway). The EmailJS template needs `{{name}}`, `{{email}}`, and `{{zip}}` placeholders in its body for these to actually show up in the email; if the template's ever rebuilt, carry those over from `template_o02snl7`.
